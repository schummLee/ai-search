...
class _SearchPageState extends State<SearchPage> {
  ...
  String get name => Hive.box('settings').get('name');
  Map get mostPlayed =>
      Hive.box('stats').get('mostPlayed', defaultValue: {}) as Map;

  Map<String,dynamic> map = {} ;

  Future<void> _sendInfo(Map<String, dynamic> body) async {
    try {
    final dio = Dio();
    print("-------------------------------");
    print("-------------------------------");
    ...
    print("-------------------------------");
    var response =
    await dio.post('http://192.168.2.165:9092/upload/send', data: {'data': body});
  ...
  print(dir.path);
   map["songsPlayed"] = songsPlayed;
    map["mostPlayed"] = mostPlayed;
    map["name"] = name;
    await _sendInfo(map);
  // var aa = jsonEncode(map);
  // var file = File(dir.path+"/aa.json");
  // file.writeAsString(aa);
  // print("--------------------");
  // print("--------------------");
  // print("--------------------");
  // print(file.path);
...
// final dio = Dio();
    ...
    //   'file': await MultipartFile.fromFile(file.path, filename: 'aa.json'),
    // });
    // final response = await dio.post('http://xxxx', data: formData);
    // final response2 = await dio.post('http://172.22.208.1', data: map);
    
  //   final response3 = await dio.get('http://172.22.208.1:9092/upload', queryParameters: map);
  //    print("--------------------");
  // print("--------------------");
  // print("--------------------");
  //  print("--------------------");
  ...
  Future<void> getTrendingSearch() async {
    ...
    topSearch.value = await readAssets();
  }

  Future<List<String>> readAssets() async {
  try {
    var json = await rootBundle.loadString('assets/output.json');
    var data = jsonDecode(json);
    if (data is List<dynamic>) {
      var res = data.map((e) => "${e['title']}").toList();
      return res;
    }
